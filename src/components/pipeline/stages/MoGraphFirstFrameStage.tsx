...
export default function MoGraphFirstFrameStage({ pipelineId, onComplete, onContinue }: MoGraphFirstFrameStageProps) {
  ...
  const handleUploadComplete = async (url: string | undefined) => {
    if (url) {
      setUploadedUrl(url);
      await updateFirstFrame({
        input: { mode: 'upload', uploaded_url: url, frame_type: 'first', style: 'motion_graphics' } as any,
      ...
      setUploadedUrl('');
      await updateFirstFrame({
        input: { mode: 'upload', uploaded_url: '', frame_type: 'first', style: 'motion_graphics' } as any,
        output: null,
        complete: false,
      });
    }
        ...
        <>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Describe your motion graphics background... e.g., 'Abstract gradient flowing from deep purple to electric blue with floating geometric shapes and subtle particle effects'"
  ...
  const outputContent = outputUrl ? (
    <div className="rounded-xl border border-border overflow-hidden">
      <img src={outputUrl} alt="Generated first frame" className="w-full object-contain" />
    </div>
  ) : null;

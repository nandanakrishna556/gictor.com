...
export default function BRollFirstFrameStage({ pipelineId, onComplete, stageNavigation }: BRollFirstFrameStageProps) {
  ...
  const saveInput = async () => {
    await updateFirstFrame({
      input: {
        ...
        // Mark this as B-Roll type for backend differentiation
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Use B-Roll specific generation (differentiates from Talking Head)
      const result = await generateBRollFirstFrame(
        pipelineId,
        { prompt, aspect_ratio: aspectRatio, reference_images: referenceImages },
        isEditing,
        previousImageUrl
      );
